...
int main(int argc, char **argv)
{
	...
	for (int i = 0; i < mode; i++) {
		if (param[i][0] == 0) {
			...
			if (i == 0)
				printf("Please set up dummy string for "
				       "check.\n");
			if (i == 1)
	...
	case 1:
		...
		model_run_single_in(
			kSingleInSeq, kSingleOutRef, har_smartphone_net_input,
	...
	case 2:
		...
		char *out_path =
			malloc(strlen(param[1]) + strlen(kOutFilePostfix) + 1);
		if (out_path == NULL) {
		...
		model_run_idx_base_to_idx_out(
			param[1], out_path, har_smartphone_net_input,
			har_smartphone_net_output, har_smartphone_preprocessing,
	...
	case 3:
		...
		model_run_acc_on_idx_base(
			param[1], param[2], har_smartphone_net_input,
			har_smartphone_net_output, har_smartphone_preprocessing,
			har_smartphone_net, NULL);
		break;
