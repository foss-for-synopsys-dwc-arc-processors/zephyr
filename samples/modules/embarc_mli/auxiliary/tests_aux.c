...
const char *test_status_to_str[] = {"PASSED", "SKIPPED", "NOT_ENOUGH_MEM_ERR",
				    "SUIT_ERROR", "FAILED"};

static void increment_data_ptr(mli_tensor *t, int value)
{
					     ...
					     uint32_t paths_num)
{
	...
	uint32_t data_buf_size =
		kMaxBufSizeToMalloc; /* Memory allocated for source data */
	...
	while (data == NULL && data_buf_size > kMinBufSizeToMalloc) {
		...
		data_buf_size =
			(data == NULL) ? data_buf_size >> 1 : data_buf_size;
	}
	...
	for (uint32_t idx = 0; idx < paths_num; idx++) {
		const uint32_t elem_size =
			mli_hlp_tensor_element_size(tensors[idx]);
		if (elem_size == 0) {
			DEBUG_BREAK;
			ret = TEST_SUIT_ERROR;
			goto ret_label;
		...
		if (elem_size * descr.num_elements >
		    tensors[idx]->data.capacity) {
			DEBUG_BREAK;
			ret = TEST_NOT_ENOUGH_MEM;
			goto ret_label;
		}
		...
		if (idx_file_read_data(&descr, NULL, tensors[idx]->shape) !=
		    IDX_ERR_NONE) {
			DEBUG_BREAK;
			ret = TEST_SUIT_ERROR;
			goto ret_label;
		...
		while (elements_accounted < total_elements) {
			...
			if (idx_file_read_data(&descr, (void *)data, NULL) !=
				    IDX_ERR_NONE ||
			    mli_hlp_float_to_fx_tensor(data, descr.num_elements,
						       tensors[idx]) !=
				    MLI_STATUS_OK) {
				DEBUG_BREAK;
				ret = TEST_SUIT_ERROR;
				     ...
				     struct ref_to_pred_output *out)
{
	...
	const float quant_scale =
		(float)((int64_t)1l << mli_hlp_tensor_scale_shift(&pred, 0)) /
	...
	while (elements_accounted < total_pred_elements) {
		...
		if (idx_file_read_data(&descr, (void *)ref_buf, NULL) !=
			    IDX_ERR_NONE ||
		    mli_hlp_fx_tensor_to_float(
			    &pred, pred_buf, data_buf_size) != MLI_STATUS_OK) {
			DEBUG_BREAK;
			ret = TEST_SUIT_ERROR;
			goto ret_label;
		}
		...
		for (uint32_t i = 0; i < descr.num_elements; i++) {
			float ref_quant =
				ref_buf[i] * quant_scale + quant_zero_offset;
			ref_quant = MAX(quant_min,
					MIN(quant_max, roundf(ref_quant))) -
				    ref_quant;

			quant_accum += ref_quant * ref_quant;
			ref_accum += ref_buf[i] * ref_buf[i];
			pred_accum += pred_buf[i] * pred_buf[i];
			noise_accum += (ref_buf[i] - pred_buf[i]) *
				       (ref_buf[i] - pred_buf[i]);
	...
	out->noise_to_quant_ratio =
		(out->noise_vec_length) / (out->quant_err_vec_length + eps);
	out->ref_to_noise_snr =
		10.f * log10f((ref_accum + eps) / (noise_accum + eps));
				    ...
				    struct ref_to_pred_output *out)
{
	...
	for (int i = 0; i < len; i++) {
		...
		noise_accum +=
			(ref_vec[i] - pred_vec[i]) * (ref_vec[i] - pred_vec[i]);
						 ...
						 mli_tensor *target_tensor)
{
	...
	if (num_vals > 1) {
		if (target_tensor->el_params.sa.scale.mem.pi16 == NULL ||
		    ...
		    target_tensor->el_params.sa.scale_frac_bits.mem.pi8 ==
			    NULL) {
			DEBUG_BREAK;
	...
	for (int i = 0; i < num_vals; i++) {
		...
		const int32_t dst_val =
			(int32_t)(mult * scale_rates[i] + round_val);
		scale_dst[i] =
			(int16_t)(MIN(MAX(dst_val, INT16_MIN), INT16_MAX));

		frac_dst[i] = scale_fraq_bits;

		const int32_t zero_val =
			(int32_t)(-zero_points[i] / scale_rates[i] + round_val);
