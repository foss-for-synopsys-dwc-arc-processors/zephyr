...
/* -------------------------------------------------------------------------- */
static void bin2val(void *out_data_, uint8_t *in_data, uint8_t type_sz,
		    uint32_t el_count)
{
	uint32_t i = 0;
	uint32_t j = 0;
	...
	case 2:
		for (; i < el_count; ++i, j += type_sz)
			((uint16_t *)out_data_)[i] = (uint16_t)(in_data[j])
							     << 8 |
						     (uint16_t)(in_data[j + 1]);
		break;
	case 4:
		for (; i < el_count; ++i, j += type_sz)
			((uint32_t *)out_data_)[i] =
				(uint32_t)(in_data[j]) << 24 |
				(uint32_t)(in_data[j + 1]) << 16 |
				(uint32_t)(in_data[j + 2]) << 8 |
				(uint32_t)(in_data[j + 3]);
		break;
	case 8:
		for (; i < el_count; ++i, j += type_sz)
			((uint64_t *)out_data_)[i] =
				(uint64_t)(in_data[j]) << 56 |
				(uint64_t)(in_data[j + 1]) << 48 |
				(uint64_t)(in_data[j + 2]) << 40 |
				(uint64_t)(in_data[j + 3]) << 32 |
				(uint64_t)(in_data[j + 4]) << 24 |
				(uint64_t)(in_data[j + 5]) << 16 |
				(uint64_t)(in_data[j + 6]) << 8 |
...
/* -------------------------------------------------------------------------- */
static void val2BEbin(uint8_t *out_data_, void *in_data, uint8_t type_sz,
		      uint32_t el_count)
{
	uint32_t i = 0;
	uint32_t j = 0;
...
enum tIdxRetVal idx_file_check_and_get_info(struct tIdxDescr *descr_)
{
	...
	if (HEADER_ELEM_SZ !=
	    fread((void *)buffer, 1, HEADER_ELEM_SZ, descr_->opened_file))
		return IDX_ERR_FILE_ACC;
	type = buffer[2];
	shapes_num = buffer[3];
	...
	for (i = 0; i < shapes_num; ++i) {
		rd_bytes += fread((void *)buffer, 1, HEADER_ELEM_SZ,
				  descr_->opened_file);
	...
	if (file_size !=
	    rd_bytes + HEADER_ELEM_SZ + (elements_num * elements_size))
		return IDX_ERR_INCORR_FILE;
...
/*
 * REMARK: If shape pointer is not NULL then read dimensions and data from file
 * beginig Else continue file reading from current position.
 */
				   ...
				   uint32_t *shape_)
{
	...
	/**
	 * If shape required than we start reading file from beginning and read
	 * shape.
	...
	if (shape_ != NULL) {
		...
		for (i = 0; i < descr_->num_dim; ++i) {
			if (fread((void *)buffer, 1, HEADER_ELEM_SZ,
				  descr_->opened_file) != HEADER_ELEM_SZ)
				return IDX_ERR_FILE_ACC;
	...
	/* ------------------- Partial reading ------------------ */
	/**
	 * total number of elemeants that will be read
	 * in this session.
	 */
	...
	while (i > 0) {
		...
		if (fread((void *)buffer, 1, count, descr_->opened_file) !=
		    count)
			return IDX_ERR_FILE_ACC;
		bin2val((void *)data_ptr, buffer, elements_size,
			elem_in_portion);
...
/**
 * Write IDX file from source (full file in 1 operation in contrast with
 * reading)
 */
enum tIdxRetVal idx_file_write(struct tIdxDescr *descr_, const void *data_,
			       uint32_t *shape_)
{
	uint32_t i, elem_in_portion;
	const uint8_t elements_size = data_elem_size(descr_->data_type);
	...
	if (fwrite((void *)buffer, 1, HEADER_ELEM_SZ, descr_->opened_file) !=
	    HEADER_ELEM_SZ)
		return IDX_ERR_FILE_ACC;
	...
	while (i > 0) {
		elem_in_portion = (i > max_elem_in_buf) ? max_elem_in_buf : i;
		val2BEbin(buffer, (void *)shape_, HEADER_ELEM_SZ,
			  elem_in_portion);

		count = elem_in_portion * HEADER_ELEM_SZ;
		if (fwrite((void *)buffer, 1, count, descr_->opened_file) !=
		    count)
			return IDX_ERR_FILE_ACC;
	...
	while (i > 0) {
		elem_in_portion = (i > max_elem_in_buf) ? max_elem_in_buf : i;
		val2BEbin(buffer, (void *)data_ptr, elements_size,
			  elem_in_portion);
				      ...
				      const uint32_t *shape_)
{
	...
	/* ---------- Set position to thex end of file ---------- */
...
enum tIdxRetVal idx_file_write_data(struct tIdxDescr *descr_, const void *data_)
{
	...
	/**
	 * Check that header has already been written, or write a placeholder
	 * for it.
	 */
