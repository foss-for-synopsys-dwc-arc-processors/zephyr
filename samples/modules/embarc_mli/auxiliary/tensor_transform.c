...
/**
 * Transform float array to MLI FX tensor according to it's element type
 * parameters
 */
				      ...
				      mli_tensor *dst)
{
	...
	float scale_val =
		(float)((int64_t)1l << mli_hlp_tensor_scale_shift(dst, 0)) /
	...
	if (dst->el_type == MLI_EL_FX_16) {
		...
		for (int idx = 0; idx < src_size; idx++) {
			...
			int32_t dst_val =
				(int32_t)(scale_val * src[idx] + round_val);
			dst_arr[idx] = (int16_t)(MIN(MAX(dst_val, INT16_MIN),
						     INT16_MAX));
		}
		...
		for (int idx = 0; idx < src_size; idx++) {
			...
			const int32_t dst_val =
				(int32_t)(scale_val * src[idx] + round_val);
			dst_arr[idx] =
				(int8_t)(MIN(MAX(dst_val, INT8_MIN), INT8_MAX));
		}
		...
		if (dst->el_params.sa.dim < 0) {
			for (int idx = 0; idx < src_size; idx++) {
				const float round_val =
					(src[idx] > 0) ? 0.5f : -0.5f;
				const int32_t dst_val =
					(int32_t)(scale_val * src[idx] +
						  round_val);
				dst_arr[idx] = (int8_t)(MIN(
			...
			const int total_elements =
				mli_hlp_count_elem_num(dst, 0);
			const int elements_to_convert =
				(concat_dim + 1 == dst->rank)
					? 1
					: mli_hlp_count_elem_num(
						  dst, concat_dim + 1);
			const int step_after_conv =
				mli_hlp_count_elem_num(dst, concat_dim);
			...
			for (int c_idx = 0; c_idx < conversions_num; c_idx++) {
				scale_val = ((int64_t)1u
					     << mli_hlp_tensor_scale_shift(
							dst, (uint32_t)c_idx)) /
					    (float)mli_hlp_tensor_scale(
						    dst, (uint32_t)c_idx);
				zero_offset =
					mli_hlp_tensor_zero_offset(dst, c_idx);
				for (int data_idx = c_idx * elements_to_convert;
				     data_idx < total_elements;
				     data_idx += step_after_conv) {
					for (int el_idx = 0;
					     el_idx < elements_to_convert;
					     ++el_idx) {
						const float round_val =
							(src[data_idx +
							     el_idx] > 0)
				      ...
				      uint32_t dst_size)
{
	...
	float scale_val =
		(float)mli_hlp_tensor_scale(src, 0) /
		(float)((int64_t)1l << mli_hlp_tensor_scale_shift(src, 0));
	...
	if (src->el_type == MLI_EL_FX_16) {
		...
		for (int idx = 0; idx < elem_num; idx++)
			dst[idx] = (float)(scale_val *
					   (src_arr[idx] - zero_offset));
			...
			const int elements_to_convert =
				(concat_dim + 1 == src->rank)
					...
					: mli_hlp_count_elem_num(
						  src, concat_dim + 1);
			const int step_after_conv =
				mli_hlp_count_elem_num(src, concat_dim);
			const int conversions_num = src->shape[concat_dim];

			for (int c_idx = 0; c_idx < conversions_num; c_idx++) {
				scale_val =
					(float)mli_hlp_tensor_scale(src,
								    c_idx) /
					(float)((int64_t)1l
						<< mli_hlp_tensor_scale_shift(
							   src, c_idx));
				zero_offset =
					mli_hlp_tensor_zero_offset(src, c_idx);
				for (int data_idx = c_idx * elements_to_convert;
				     data_idx < total_elements;
				     data_idx += step_after_conv) {
					for (int el_idx = 0;
