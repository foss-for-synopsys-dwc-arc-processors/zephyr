...
def shell(dut: DeviceAdapter) -> Shell:
    ...
    # iotdk: After 45s boot wait, data appears around 9-10s, full boot takes 50-60 seconds
    # Other ARC boards: After 20s boot wait, may need up to 60s for prompt on some boards
    build_dir_str = str(dut.device_config.build_dir).lower()
    is_very_slow_board = 'iotdk' in build_dir_str
    timeout = 60 if is_very_slow_board else 60
