...
uint32_t z_arc_ss_call_invoke6(uint32_t arg1, uint32_t arg2, uint32_t arg3,
			   uint32_t arg4, uint32_t arg5, uint32_t arg6,
			   uint32_t call_id)
{
	register uint32_t ret __asm__("r0") = arg1;
	...
	__asm__ volatile(
			 ...
			 "pop %%blink\n"
			 : "=r"(ret)
			 : [id] "i" (SJLI_CALL_ARC_SECURE_SERVICE),
			   "r" (ret), "r" (r1), "r" (r2), "r" (r3),
			   "r" (r4), "r" (r5), "r" (r6));
