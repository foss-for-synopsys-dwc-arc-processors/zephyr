...
static inline void ss_region_init(uint32_t index, uint32_t region_addr, uint32_t size,
...
static inline void ss_region_set_attr(uint32_t index, uint32_t attr)
{
	z_arc_s_call_invoke6(index, 0, 0, attr, SS_MPU_OP_WRITE_ATTR,
			     0, ARC_S_CALL_MPU);
}

static inline uint32_t ss_region_get_attr(uint32_t index)
{
	return z_arc_s_call_invoke6(index, 0, 0, 0, SS_MPU_OP_READ_ATTR,
				    0, ARC_S_CALL_MPU);
}

static inline uint32_t ss_region_get_start(uint32_t index)
{
	return z_arc_s_call_invoke6(index, 0, 0, 0, SS_MPU_OP_READ_START,
				    0, ARC_S_CALL_MPU);
}

static inline void ss_region_set_start(uint32_t index, uint32_t start)
{
	z_arc_s_call_invoke6(index, start, 0, 0, SS_MPU_OP_WRITE_START,
			     0, ARC_S_CALL_MPU);
}

static inline uint32_t ss_region_get_end(uint32_t index)
{
	return z_arc_s_call_invoke6(index, 0, 0, 0, SS_MPU_OP_READ_END,
				    0, ARC_S_CALL_MPU);
}

static inline void ss_region_set_end(uint32_t index, uint32_t end)
{
 ...
 */
static inline int ss_mpu_probe(uint32_t addr)
{
