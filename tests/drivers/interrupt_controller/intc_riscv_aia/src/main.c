...
ZTEST(intc_riscv_aia, test_aplic_target_encoding)
{
	...
	uint32_t target_val = ((2 & APLIC_TARGET_HART_MASK) << APLIC_TARGET_HART_SHIFT) |
	                      ...
	                      (65 & APLIC_TARGET_EIID_MASK);
	/* Expected: 0x00080841 = (2 << 18) | BIT(11) | 65 */
...
ZTEST(intc_riscv_aia, test_aplic_genmsi_encoding)
{
	...
	uint32_t genmsi_val = ((1 & APLIC_GENMSI_HART_MASK) << APLIC_GENMSI_HART_SHIFT) |
	                      ...
	                      (70 & APLIC_GENMSI_EIID_MASK);
	/* Expected: 0x00040846 = (1 << 18) | BIT(11) | 70 */
...
ZTEST(intc_riscv_aia, test_imsic_eidelivery_modes)
{
	...
	zassert_equal(0x20000000, EIDELIVERY_MODE_DMSI, "DMSI mode (01)");
	zassert_equal(0x40000000, EIDELIVERY_MODE_DDI, "DDI mode (10)");
	zassert_equal(0x60000000, EIDELIVERY_MODE_BOTH, "Both modes (11)");
	...
	/* Test building EIDELIVERY value for DMSI mode */
	uint32_t eidelivery_dmsi = EIDELIVERY_ENABLE | EIDELIVERY_MODE_DMSI;
	zassert_equal(0x20000001, eidelivery_dmsi, "EIDELIVERY DMSI enabled");
}
...
ZTEST(intc_riscv_aia, test_aia_msi_routing_encoding)
{
	...
	/* Expected: (1 << 18) | BIT(11) | 65 = 0x00040841 */
...
ZTEST(intc_riscv_aia, test_eiid_range_boundaries)
{
	...
	uint32_t eiid_overflow = 0xFFFFFFFF;
...
ZTEST(intc_riscv_aia, test_hart_index_boundaries)
{
	...
	uint32_t hart_overflow = 0xFFFFFFFF;
	uint32_t encoded_overflow = (hart_overflow & APLIC_TARGET_HART_MASK) << APLIC_TARGET_HART_SHIFT;
