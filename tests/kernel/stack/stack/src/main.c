...
static void thread_entry_fn_single(void *p1, void *p2, void *p3)
{
	...
	zassert_false(memcmp(tmp, data1, STACK_LEN * sizeof(stack_data_t)),
...
static void thread_entry_fn_dual(void *p1, void *p2, void *p3)
{
	...
	zassert_false(memcmp(tmp, data2, STACK_LEN * sizeof(stack_data_t)),
		      "Push & Pop items does not match");
}
...
static void thread_entry_fn_isr(void *p1, void *p2, void *p3)
{
	...
	zassert_false(memcmp(data_isr, data2, STACK_LEN * sizeof(stack_data_t)),
		      "Push & Pop items does not match");
...
static void test_isr_stack_play(void)
{
	...
	zassert_false(memcmp(data_isr, data1, STACK_LEN * sizeof(stack_data_t)),
		      "Push & Pop items does not match");
