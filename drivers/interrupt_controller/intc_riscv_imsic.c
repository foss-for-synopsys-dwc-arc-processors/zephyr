...
static int imsic_init(const struct device *dev)
{
	...
	LOG_DBG("  EIDELIVERY=0x%08lx EITHRESHOLD=0x%08lx",
		(unsigned long)icsr_read(ICSR_EIDELIVERY),
		(unsigned long)icsr_read(ICSR_EITHRESH));
...
{
	uint32_t topei = csr_read(CSR_MTOPEI);
...
void z_riscv_imsic_secondary_init(void)
{
	...
	if (!(eidelivery_readback & EIDELIVERY_ENABLE)) {
		LOG_ERR("CPU %u IMSIC EIDELIVERY enable bit not set! Got 0x%08lx",
