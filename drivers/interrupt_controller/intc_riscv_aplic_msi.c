...
int riscv_aplic_msi_global_enable(const struct device *dev, bool enable)
{
    ...
    /* Read back to verify */
    uint32_t readback = rd32(cfg->base, APLIC_DOMAINCFG);
    LOG_INF("APLIC DOMAINCFG: wrote 0x%08x, read 0x%08x", v, readback);
...
int riscv_aplic_inject_software_interrupt(const struct device *dev, uint32_t eiid, uint32_t hart_id, uint32_t context)
{
     ...
     */
    uint32_t genmsi_val = ((hart_id & APLIC_GENMSI_HART_MASK) << APLIC_GENMSI_HART_SHIFT) |
                          ((context & APLIC_GENMSI_CONTEXT_MASK) << APLIC_GENMSI_CONTEXT_SHIFT) |
                          APLIC_GENMSI_MMSI_MODE |
                          (eiid & APLIC_GENMSI_EIID_MASK);
...
static int aplic_init(const struct device *dev)
{
    ...
    LOG_INF("APLIC: Got IMSIC address from DT msi-parent: 0x%08x", imsic_addr);
    ...
    uint32_t msi_geom = (lhxs << APLIC_MSIADDRCFGH_LHXS_SHIFT) |
                        (lhxw << APLIC_MSIADDRCFGH_LHXW_SHIFT) |
                        (hhxs << APLIC_MSIADDRCFGH_HHXS_SHIFT) |
                        (hhxw << APLIC_MSIADDRCFGH_HHXW_SHIFT);

    LOG_INF("SMP MSI geometry: num_harts=%u, LHXS=%u, LHXW=%u, HHXS=%u, HHXW=%u",
            num_harts, lhxs, lhxw, hhxs, hhxw);
    ...
    if (msi_low == 0 && msi_high == 0) {
        LOG_INF("MSI address registers uninitialized, configuring...");
        ...
        LOG_INF("MSI address registers pre-configured (read-only interface)");
    }

    uint32_t smsi_low = rd32(cfg->base, APLIC_SMSIADDRCFG);
    uint32_t smsi_high = rd32(cfg->base, APLIC_SMSIADDRCFGH);

    LOG_INF("APLIC MSI address configuration:");
    LOG_INF("  Expected IMSIC: 0x%08x (PPN: 0x%08x)", imsic_addr, imsic_ppn);
    LOG_INF("  MSIADDR:  0x%08x%08x", msi_high, msi_low);
    LOG_INF("  SMSIADDR: 0x%08x%08x", smsi_high, smsi_low);

    /* Enable MSI mode + IE in DOMAINCFG */
    riscv_aplic_msi_global_enable(dev, true);

    LOG_INF("APLIC MSI init complete at 0x%lx, sources=%u",
            (unsigned long)cfg->base, cfg->num_sources);
    return 0;
}
...
#define APLIC_INIT(inst) \
    ...
    DEVICE_DT_INST_DEFINE(inst, aplic_init, NULL, \
        &aplic_data_##inst, &aplic_cfg_##inst, \
